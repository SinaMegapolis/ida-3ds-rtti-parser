...
class BasicClass:
    ...
    def read_name(self):
        logger.warning(f'Current pointer pos: {hex(self.stream.get_current_position())}')
    ...
    def retype_vtable_function(self, typename, func_ea, func_name):
        ...
        # rename function
        func = ida_funcs.get_func(func_ea) #Ensure that the address whose name gets changed is the actual start address
        if func is not None:
            idc.set_name(func.start_ea, new_name)
        # apply type name to function
        if make_class_method(func_ea, typename):
            logger.warning(f'Applied signature to {func_name}')
